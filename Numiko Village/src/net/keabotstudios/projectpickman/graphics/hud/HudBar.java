...
public class HudBar extends HudObject {
	...
	public void render(Graphics2D g) {
		g.drawImage(barFrame[0], x - 1, y - 1, null);
		for(int i = 0; i < width; i++) {
			g.drawImage(barFrame[1], x + i, y - 1, null);
		}
		g.drawImage(barFrame[2], x + width, y - 1, null);
		
		int barWidth =  (int) (bar.getWidth() * ((float) value / (float) maxValue));
		if(barWidth != 0) g.drawImage(bar.getSubimage(0, 0, barWidth, bar.getHeight()), x, y, null);
	}
